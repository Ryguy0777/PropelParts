#include <kamek.h>
#include <propelparts/bases/d_a_en_pata_kuribon.hpp>
...
CUSTOM_ACTOR_PROFILE(EN_PATA_KURIBON, daEnPataKuribon_c, fProfile::EN_KURIBO, fProfile::DRAW_ORDER::EN_KURIBO, 0x12);

STATE_DEFINE(daEnPataKuribon_c, Dizzy);
STATE_DEFINE(daEnPataKuribon_c, Carry);
STATE_DEFINE(daEnPataKuribon_c, Throw);
STATE_DEFINE(daEnPataKuribon_c, Kick);
STATE_DEFINE(daEnPataKuribon_c, Recover);
...
const float daEnPataKuribon_c::smc_KICK_SPEED_X = 2.5f;
const float daEnPataKuribon_c::smc_KICK_SPEED_Y = 2.0f;

const s16 l_pata_kuribon_carry_angleY[] = { -0x6000, 0x6000 };

daEnPataKuribon_c::daEnPataKuribon_c() {
    mHasWings = true;
}

bool daEnPataKuribon_c::isSpinLiftUpEnable() {
    return isState(StateID_Dizzy);
}

void daEnPataKuribon_c::setSpinLiftUpActor(dActor_c *carryingActor) {
    mCarryID = carryingActor->getPlrNo();
    mCarryPos.set(0.0f, 0.0f, 0.0f);
    changeState(StateID_Carry);
}

void daEnPataKuribon_c::Normal_VsEnHitCheck(dCc_c *self, dCc_c *other) {
    ...
    if (self->mCcData.mAttack == CC_ATTACK_SHELL && (flag & 0x100) && hitCallback_Shell(self, other)) {
        ...
        other->mInfo |= CC_NO_HIT;
    } else if (isState(StateID_Walk) || isState(StateID_Pata_Walk)) {
...
void daEnPataKuribon_c::Normal_VsPlHitCheck(dCc_c *self, dCc_c *other) {
...
void daEnPataKuribon_c::Normal_VsYoshiHitCheck(dCc_c *self, dCc_c *other) {
...
void daEnPataKuribon_c::initializeState_DieOther() {
    return dEn_c::initializeState_DieOther();
}

void daEnPataKuribon_c::executeState_DieOther() {
    return dEn_c::executeState_DieOther();
}

void daEnPataKuribon_c::finalizeState_DieOther() {
...
void daEnPataKuribon_c::reactFumiProc(dActor_c *player) {
    if (mHasWings) {
        mHasWings = false;
        setWalkSpeed();
...
void daEnPataKuribon_c::createBodyModel() {
...
void daEnPataKuribon_c::createOtherModel() {
    mWingRes = dResMng_c::m_instance->getRes("kuribon_wing", "g3d/wing.brres");
    nw4r::g3d::ResMdl wmdl = mWingRes.GetResMdl("wing");
    mWingModel.create(wmdl, &mAllocator, 0x20, 1, nullptr);
    setSoftLight_Enemy(mWingModel);

    nw4r::g3d::ResAnmChr resAnmChr = mWingRes.GetResAnmChr("wing_kuri");
    mWingAnm.create(wmdl, resAnmChr, &mAllocator, 0);
}
...
void daEnPataKuribon_c::setWalkAnm() {
    nw4r::g3d::ResAnmChr resAnmChr = mRes.GetResAnmChr("walk");
...
void daEnPataKuribon_c::playWalkAnm() {
    mModel.play();
}

void daEnPataKuribon_c::updateCarryCc() {
...
void daEnPataKuribon_c::setDeathInfo_Carry(dActor_c *killedBy) {
...
void daEnPataKuribon_c::initializeState_Dizzy() {
    ...
    mAngle.x = 0x8000;
    mAngle.y = l_pata_kuribon_carry_angleY[mDirection];
    mSpeed.x = 0.0f;
    mAccelY = -0.1875f;
    mSpeedMax.y = -4.0f;
}

void daEnPataKuribon_c::finalizeState_Dizzy() {};
...
void daEnPataKuribon_c::initializeState_Carry() {
    mPlayerNo = mCarryID;
...
void daEnPataKuribon_c::finalizeState_Carry() {
    dAcPy_c *player = daPyMng_c::getPlayer(mCarryID);
...
void daEnPataKuribon_c::initializeState_Throw() {
...
void daEnPataKuribon_c::finalizeState_Throw() {
    clrComboCnt();
    mAccelF = 0.0f;
    mSpeedMax.x = 0.0f;
    mCc.mCcData.mAttack = CC_ATTACK_NONE;
...
void daEnPataKuribon_c::initializeState_Kick() {
    mAnmChr.setRate(2.0f);
...
void daEnPataKuribon_c::finalizeState_Kick() {
    clrComboCnt();
    mCc.mCcData.mAttack = CC_ATTACK_NONE;
}
...
void daEnPataKuribon_c::initializeState_Recover() {
...
void daEnPataKuribon_c::finalizeState_Recover() {}
